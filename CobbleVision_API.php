...
function deleteMediaFile(IDArray){
  try{
    ...
    $res = $client -> request("POST", ($this -> BaseURL) + $endpoint + "?id=" + json_encode(IDArray), ["headers" => array("Accept" => "application/json", "Content-Type" => "application/json"), "auth" => array($this -> apiUserName => $this -> apiPassword)])
